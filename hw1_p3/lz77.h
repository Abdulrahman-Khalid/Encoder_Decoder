...
public:
  ...
  int decorrelation(const int &width, const vector<unsigned char> &msg, int S, int T, int A, vector<Triplet> *encoded_msg);

  void setPixelCount(unsigned int pixelCount);

private:
  // Any data members here ...
  vector<Triplet> vTags; // pointer used to make arr of Triplets
  unsigned int pixelCount;
  Triplet findLongestMatch(const vector<unsigned char> &msg, int separator, int S, int T);
};
